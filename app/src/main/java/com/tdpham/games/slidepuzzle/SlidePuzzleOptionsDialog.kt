...
object SlidePuzzleOptionsDialog {
    private const val PREFS_NAME = "slide_puzzle_settings"
    ...
    fun show(context: Context, onDismiss: () -> Unit) {
        ...
        BaseOptionsDialog(context)
            .setTitle(context.getString(R.string.slide_puzzle_settings_title))
            .addOption(
                label = context.getString(R.string.snake_level_label),
                valueProvider = {
                    val index = prefs.getInt(KEY_DIFFICULTY, 0)
                    context.getString(when(index) {
                        0 -> R.string.slide_puzzle_level_1
                        1 -> R.string.slide_puzzle_level_2
                        else -> R.string.slide_puzzle_level_3
                    })
                },
                descProvider = {
                    context.getString(R.string.game_slide_puzzle_instructions).split("\n").first()
                },
                onClick = {
                    val index = prefs.getInt(KEY_DIFFICULTY, 0)
