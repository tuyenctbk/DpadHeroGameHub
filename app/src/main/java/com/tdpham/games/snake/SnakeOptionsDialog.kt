...
object SnakeOptionsDialog {
    ...
    fun show(context: Context, onDismiss: () -> Unit) {
        ...
        BaseOptionsDialog(context)
            .setTitle(context.getString(R.string.snake_settings_title))
            .addOption(
                label = context.getString(R.string.snake_level_label),
                valueProvider = {
                    val index = prefs.getInt(KEY_DIFFICULTY, 1)
                    context.getString(when(index) {
                        0 -> R.string.snake_level_1
                        2 -> R.string.snake_level_3
                        else -> R.string.snake_level_2
                    })
                },
                descProvider = {
                    val index = prefs.getInt(KEY_DIFFICULTY, 1)
                    context.getString(when(index) {
                        0 -> R.string.snake_level_1_desc
                        2 -> R.string.snake_level_3_desc
                        else -> R.string.snake_level_2_desc
                    })
                },
                onClick = {
                    val index = prefs.getInt(KEY_DIFFICULTY, 1)
